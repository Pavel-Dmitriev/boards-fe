...
export default function CommentsSection({ comments, isLoading }: ICommentsSectionProps) {
  return (
    <section className="mx-1 mb-4 max-w-[98.5%] border-b border-gray-300 pb-4">
      <div className="mb-4 flex items-center justify-between">
        <h3 className="font-semibold">Комментарии</h3>

        <span className="mr-3 inline-block rounded-full bg-violet-100 px-3 py-1 text-sm font-semibold text-gray-600">
          {comments.length}
        </span>
      </div>
      ...
      {!isLoading && comments.length === 0 ? (
        <p className="py-4 text-center text-gray-400">Нет комментариев</p>
      ) : (
